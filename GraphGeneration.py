...
# =============================================================================
# PreStep 1: Load parsered dataset HDFS
# =============================================================================
import pandas as pd
...
raw_df = df[["LineId","EventId","GroupId", "EventTemplate"]] #11175629
...
with open('/Users/zlifr/Desktop/OCDIGCN/Data/Gloves/Results/EmbeddingDict.json', 'r') as fp:
...
def GraphConstruction(my_example_df, graph_index, my_node_accum, new_node_accum):
    # =============================================================================
    # #write a function to generate a graph from each group of log events and store them as
    # 1. HDFS_A.txt
    # 2. HDFS_edge_attributes.txt
    # 3. HDFS_graph_indicator.txt
    # 4. HDFS_graph_labels.txt
    # 5. HDFS_node_attributes.txt
    # =============================================================================
    ...
    df_graph_indicator["indicator"] = [graph_index+1]*len(new_node_accum)
    ...
    label_value = df_label.iloc[graph_index]['Label']
    # print("\n------label------")
    # print(label_value)
...
def ConcatGraphs(ReadGraph, graph_index, my_node_accum, new_node_accum, adj_type):
    # =============================================================================
...
event_to_check = list(all_event_df["BlockId"])

list_group = event_to_check[0:10000]
...
for group_name in tqdm(list_group):
    ...
    GraphConstruction(my_example_df = example_df,
                      graph_index = count_index,
                      my_node_accum = node_accum,
                      new_node_accum = new_event_list)
    
    ##after generating each graph, we get its appr adj matrix accordingly
    MyReadGraph = read_tu_data(folder = r"/Users/zlifr/Desktop/OCDIGCN/Data/HDFS/Graph/TempRaw",
                                prefix = MyDataName,
                                adj_type = 'ib')
    
    ##oncatenate all appr graphs 
